...
\begin{figure}[tb]
    ...
    \begin{quantikz}[row sep=0.5cm, column sep=0.5cm]
        \lstick{$q_0$} & \gate{R_x(x_0)} & \gate{Rot(\theta_0\sim\theta_2)} & \ctrl{1} & \gate{R_x(x_0)} & \gate{Rot(\theta_{16}\sim\theta_{18})} & \ctrl{1} & \meter{} \\
        \lstick{$q_1$} & \gate{R_x(x_1)} & \gate{Rot(\theta_3\sim\theta_5)} & \targ{}   & \gate{R_x(x_1)} & \gate{Rot(\theta_{19}\sim\theta_{21})} & \targ{}   & \meter{} \\
        \lstick{$q_2$} & \gate{R_x(x_2)} & \gate{Rot(\theta_6\sim\theta_8)} & \ctrl{1} & \gate{R_x(x_2)} & \gate{Rot(\theta_{22}\sim\theta_{24})} & \ctrl{1} & \meter{} \\
        \lstick{$q_3$} & \gate{R_x(x_3)} & \gate{Rot(\theta_9\sim\theta_{11})} & \targ{}   & \gate{R_x(x_3)} & \gate{Rot(\theta_{25}\sim\theta_{27})} & \targ{}   & \meter{} \\
    \end{quantikz}    
    \caption{\centering 上图：QuantumMLP 网络结构图：线性映射 + 参数化量子电路 + 测量输出\\ 下图：参数化量子电路}
