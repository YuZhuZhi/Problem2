...
\centering
\begin{quantikz}
\lstick{$\ket{0}$} & \gate{RX(x_1)} & \qw \\
\lstick{$\ket{0}$} & \gate{RX(x_2)} & \qw \\
& \vdots & \\
\lstick{$\ket{0}$} & \gate{RX(x_N)} & \qw
\end{quantikz}
\caption{量子数据编码电路}
\label{fig:qencode}
\end{figure}
...
\centering
\begin{quantikz}
\lstick{$\ket{\psi_1}$} & \ctrl{1} & \gate{RX(\theta_1)} & \qw \\
\lstick{$\ket{\psi_2}$} & \targ{} & \gate{RY(\theta_2)} & \qw
\end{quantikz}
\caption{参数化纠缠层电路}
\label{fig:entangle}
\end{figure}
